...
def train(cfg):
    ...
    if cfg.MODEL.IF_WITH_CENTER == 'no':
        ...
        if cfg.MODEL.PRETRAIN_CHOICE == 'self':
            ...
            ### YT TEST
            param_dict = torch.load(cfg.MODEL.PRETRAIN_PATH).state_dict()
        ...
        if cfg.MODEL.PRETRAIN_CHOICE == 'self':
            ...
            # Done Internally maybe using MODEL.FREEZE parameter -> For instance, I remove thess lines
            # YT
            ...
            # optimizer.load_param(path_to_optimizer)
            # YT TEST : DO NOT LOAD OPTIMIZER
            optimizer = make_optimizer(cfg, model)
            #optimizer.load_state_dict(torch.load(path_to_optimizer))
            # YT TEST
            # center_criterion = torch.load(path_to_center_param)

            # YT TEST : DO NOT LOAD CENTER PARAM
            #center_criterion.load_state_dict(torch.load(path_to_center_param))
            #center_criterion.to(cfg.MODEL.DEVICE)
            # YT TEST
            # optimizer_center = torch.load(path_to_optimizer_center)
            # YT TEST : DO NOT LOAD OPTIMIZER CENTER
